...
class TaskScheduler(Base):
    ...
    name: Mapped[str] = mapped_column(UniversalStr(64), unique=True, comment='任务名称')
    task: Mapped[str] = mapped_column(UniversalStr(256), comment='要运行的 Celery 任务')
    ...
    queue: Mapped[str | None] = mapped_column(UniversalStr(256), comment='CELERY_TASK_QUEUES 中定义的队列')
    exchange: Mapped[str | None] = mapped_column(UniversalStr(256), comment='低级别 AMQP 路由的交换机')
    routing_key: Mapped[str | None] = mapped_column(UniversalStr(256), comment='低级别 AMQP 路由的路由密钥')
    ...
    interval_period: Mapped[str | None] = mapped_column(UniversalStr(256), comment='任务运行之间的周期类型')
    crontab: Mapped[str | None] = mapped_column(UniversalStr(64), default='* * * * *', comment='Crontab 表达式')
