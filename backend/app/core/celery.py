...
celery_app.conf.broker_url = (
    ...
    f'{settings.CELERY_REDIS_PORT}/{settings.CELERY_REDIS_DATABASE_BROKER}'
)
celery_app.conf.result_backend = (
    f'redis://:{settings.CELERY_REDIS_PASSWORD}@{settings.CELERY_REDIS_HOST}:'
    f'{settings.CELERY_REDIS_PORT}/{settings.CELERY_REDIS_DATABASE_BACKEND}'
)
celery_app.conf.result_backend_transport_options = {
    'global_keyprefix': settings.CELERY_REDIS_BACKEND_PREFIX,
    'retry_policy': {'timeout': settings.CELERY_REDIS_BACKEND_TIMEOUT},
