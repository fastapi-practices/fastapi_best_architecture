...
class CRUDGen:
    ...
    async def get_all_tables(db: AsyncSession, table_schema: str) -> Sequence[str]:
        t = text(
            ...
            'where table_name not like "sys_gen_%" and table_schema = :table_schema;'
    ...
    async def get_table(db: AsyncSession, table_name: str) -> Row[tuple]:
        t = text(
            ...
            'where table_name not like "sys_gen_%" and table_name = :table_name;'
    ...
    async def get_all_columns(db: AsyncSession, table_name: str) -> Sequence[Row[tuple]]:
        t = text(
            ...
            'from information_schema.columns where table_name = :table_name and column_name != "id" '
            'order by sort;'
        ).bindparams(table_name=table_name)
        stmt = await db.execute(t)
