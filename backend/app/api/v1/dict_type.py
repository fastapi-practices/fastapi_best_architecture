...
@router.get('', summary='（模糊条件）分页获取所有字典类型', dependencies=[Depends(jwt_auth), PageDepends])
...
@router.post('', summary='创建字典类型', dependencies=[Depends(RBAC.rbac_verify)])
async def create_dict_type(obj: CreateDictType):
    await DictTypeService.create(obj=obj)
    return await response_base.success()


@router.put('/{pk}', summary='更新字典类型', dependencies=[Depends(RBAC.rbac_verify)])
...
@router.delete('', summary='（批量）删除字典类型', dependencies=[Depends(RBAC.rbac_verify)])
